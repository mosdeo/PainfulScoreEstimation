...
namespace LKY
{
  public class Standardizer
  {
    ...
    String strStdDev = "public static readonly double[] PainFace_1024_DA_StdErr = new double[] {";
    String strMeans = "public static readonly double[] PainFace_1024_DA_Avg = new double[] {";
    for (int i = 0; i < stdDevs.Length; ++i)
    {
        if (i == stdDevs.Length - 1)
        {
            strStdDev += this.stdDevs[i].ToString() + "};";
            strMeans += this.means[i].ToString() + "};";
        }
        else
        {
            strStdDev += this.stdDevs[i].ToString() + ",";
